...
public class Store implements Datastore {
    ...
    public long write(HttpServletRequest request, String id, long offset, long max)
        ...
        if (flen < offset) {
            log.error(file.getAbsolutePath() + " with length:" + flen + " but offset:" + offset + "+" + max);
        }
        // TODO: check that file offset matches request offset.

        long transferred = 0L;
        String path = getBinPath(id);
        FileChannel dest;
        RandomAccessFile raf = null;
        FileOutputStream fos = null;
        if (false) {
            raf = new RandomAccessFile(path, "rw"); // throws if file doesn't exist
            //TODO: is it mapped? can we just use FileOutputStream?
            dest = raf.getChannel();
        } else {
        ...
        try {
            log.debug("Calling FileChannel.transferFrom " + offset + "+" + max);
    ...
    @Override
    public boolean finish(FileInfo fi) throws Exception {
        //"SHA256" will give SHA256 MessageDigest not available
        MessageDigest md = MessageDigest.getInstance("SHA-256");
        File f = new File(getBinPath(fi.id));
        BufferedInputStream bis = new BufferedInputStream(new FileInputStream(f)); // throws if file doesn't exist
        byte[] bytes = new byte[4096];
        while (true) {
            int len = bis.read(bytes);
            if (len == -1) {
                byte[] hash = md.digest();
                String hash_s = HexUtils.toHexString(hash).toUpperCase();
                fi.sha256Checked = fi.id.equals(hash_s);
                if (fi.sha256Checked) {
                } else {
                    boolean deleted = f.delete();
                    log.error(fi.id + " sha256 are not same.deleted:" + deleted + " :" + hash_s);
                    log.error(" sha256 of empty string:" + HexUtils.toHexString(md.digest()));
                }
                return fi.sha256Checked;
            }
